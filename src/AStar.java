...
public class AStar {
    ...
    //allow to calculate diagonal path
    Boolean allowDiagonalPath = false;

    Comparator comparator;

    List findPath(boolean [][] walkableMask,int startX, int startY, int endX, int endY,Boolean allowDiagonalPath) {
        List path = new ArrayList(300); //create empty arraylist for path nodes
        if(startX == endX && startY == endY){ // check if startfield is also destinationfield
            path.add(new Node(endX,endY)); // add destinationfield to path
            return  path;
        }

        this.allowDiagonalPath = allowDiagonalPath;

        path = getPath(walkableMask,startX, startY, endX, endY); // calculate path
        return path;
    }

    private List getPath(boolean [][] walkableMask, int startX, int startY, int endX, int endY) {
        start = new Node(startX, startY); // create start node
        end = new Node(endX, endY); // create end node
        addNode(walkableMask, start);
